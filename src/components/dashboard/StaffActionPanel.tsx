      ...
      {highCongestionLocations.map((loc) => {
        ...
        return (
          <GlassCard
            key={loc.id}
            glow="red"
            className="p-5 border-rose-500/40 bg-slate-900/90"
          >
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-slate-400">({loc.peopleCount} people)</span>
                  </div>
                  <h3 className="font-extrabold text-base text-white">{loc.name}</h3>
                </div>
              </div>

              <span className="text-xs font-mono text-rose-300 bg-rose-500/10 border border-rose-500/30 px-2.5 py-1 rounded-full">
                Wait: {loc.estimatedWaitMin} min
              </span>
            </div>

            {/* AI Operational Suggestion */}
            <div className="p-3 rounded-lg bg-slate-950/80 border border-white/10 mb-4">
              <p className="text-xs text-teal-300 font-semibold mb-1 flex items-center gap-1.5">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-white/10">
              {actionDone ? (
                <div className="flex items-center gap-2 text-xs text-emerald-400 font-medium bg-emerald-500/10 px-3 py-1.5 rounded-lg border border-emerald-500/30">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-slate-800 text-slate-200 hover:bg-slate-700 border border-white/10 transition-colors"
                  >
                  ...
                  <Link
                    href={`/queue/${loc.id}`}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-slate-900 text-slate-300 hover:text-white border border-white/10 transition-colors"
                  >
